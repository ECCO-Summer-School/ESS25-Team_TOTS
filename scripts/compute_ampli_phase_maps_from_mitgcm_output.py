...
data_dir ='/home/plombat/Documents/Postdoc/ECCO/ETAN_hourly' #CHANGE: directory of ETAN files
grid_dir ='/home/plombat/Documents/Postdoc/ECCO/grid_1deg' #CHANGE: directory of grid files (e.g. XC.data)
...
ds_llc['time'] = xr.date_range(start='1992-01-01T13:00:00', periods=ds_llc.sizes['time'], freq='h')#,use_cftime=True) #dims
...
if CLIM: # Remove monthly climatology
...
def loop_tiles(ds):
    for tilenum in (range(ntile)):
        ...
        if CLIM:
            np.savetxt('./maps_mitgcm_5years_clim/amp_consts_'+str(tilenum)+'.txt', amp_2d) #CHANGE SAVE PATH
            np.savetxt('./maps_mitgcm_5years_clim/pha_consts_'+str(tilenum)+'.txt', pha_2d)
        else: 
            np.savetxt('./maps_mitgcm_5years_noclim/amp_consts_'+str(tilenum)+'.txt', amp_2d) #CHANGE SAVE PATH
            np.savetxt('./maps_mitgcm_5years_noclim/pha_consts_'+str(tilenum)+'.txt', pha_2d)

if CLIM:
    loop_tiles(ds_llc_anom)
else: 
    loop_tiles(ds_llc)
