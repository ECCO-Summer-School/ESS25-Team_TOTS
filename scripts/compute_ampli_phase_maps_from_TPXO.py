...
model = pyTMD.io.model('../../TMD', #TOCHANGE: path of the TPXO data
                       compressed= False
                       ).elevation('TPXO9.1')
...
def make_map():  
    ...
    lon2d,lat2d = np.meshgrid(longitude,latitude) # We want to treat the whole map at once, but the function only takes 
    #longitude and latitude arrays of equal size
    lonflat = lon2d.ravel() #packs 2d array in 1d array
    latflat= lat2d.ravel() 
    amp,ph,D,components = pyTMD.io.OTIS.extract_constants(lonflat,latflat, model.grid_file, model.model_file, projection=4326,
                                                 type='z',
                                                 method='bilinear',
                                                 grid='OTIS')
    ny, nx = lon2d.shape     # e.g. (180, 180)
    n_constituents = amp.shape[1]  # should be 15 constituents
    ...
    for c in range(len(components)): #CHANGE the writing path 
        np.savetxt('./maps_tpxo_long_180/ampli_ll_txpo_'+str(components[c])+'.txt',amp_grid[:,:,c])
        np.savetxt('./maps_tpxo_long_180/ph_ll_txpo_'+str(components[c])+'.txt',ph_grid[:,:,c])
make_map()
